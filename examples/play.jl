...
const env = RCW.SingleRoom(T = T, rng = rng)
...
function keyboard_callback(window, key, mod, isPressed)::Cvoid
    if isPressed
        ...
        push!(image_sequence, RCW.get_combined_view(env))
...
function play(env::RCW.SingleRoom)
    cv = RCW.get_combined_view(env)
    push!(image_sequence, RCW.get_combined_view(env))
    ...
    permutedims!(fb_cv, RCW.get_combined_view(env), (2, 1))
...
const gif = zeros(UInt32, size(RCW.get_combined_view(env))..., length(image_sequence))
